...
{
    public class PCTriggerCollisionWeapon : MonoBehaviour
    {
        public void Start()
        {
            collisionEffects = GetComponentsInChildren<CollisionWeaponEffect>();
            if (meleeWeapon == null && GetComponent<MeleeWeapon>())
            {
        ...
        public void OnTriggerEnter(Collider col)
        {
            ...
            {
                effect.DoEffect(col.transform, new Collision());
            }
            ...
            if (GetComponent<CollisionSound>())
            {
                DoEffect(col.transform, col, 100f);
            }
        ...
        public void DoEffect(Transform hitTransform, Collider col, float impact)
        {
            ...
            if (GetComponent<CollisionSound>().onlySoundOnRig && !col.attachedRigidbody)
            {
                return;
            }
            if (meleeWeapon)
