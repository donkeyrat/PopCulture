...
public class SpawnBone : MonoBehaviour, IRemotelyControllable
{
	...
	public void SpawnTowardsTarget()
	{
		...
		{
			StartCoroutine(SpawnUnit(base.transform.position, Quaternion.LookRotation((data.targetData.mainRig.position - transform.position).normalized + Random.insideUnitSphere * spread * 0.01f)));
		}
	}
	...
	public void SpawnTowardsTargetWithoutY()
	{
		...
		if ((bool)data.targetData)
		{
			StartCoroutine(SpawnUnit(rotation: Quaternion.LookRotation(new Vector3(data.targetData.mainRig.position.x - data.mainRig.position.x, 0f, data.targetData.mainRig.position.z - data.mainRig.position.z).normalized + Random.insideUnitSphere * spread * 0.01f), position: base.transform.position));
		}
	}
