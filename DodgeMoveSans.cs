...
public class DodgeMoveSans : Move, IRemotelyControllable
{
	private DataHandler target;
	...
	public void DoMove()
	{
		...
		DoMove(null, data.targetMainRig, target);
	}
	...
	public override void DoMove(Rigidbody enemyWeapon, Rigidbody enemyTorso, DataHandler targetData)
	{
		...
		for (int i = 0; i < moves.Length; i++)
		{
			...
			{
				StartCoroutine(DoMoveSequence(move, enemyWeapon, enemyTorso, target));
			}
		}
	}
			...
			if (CheckConditions())
			{
				for (int j = 0; j < rigs.Count; j++)
				{
					...
					if (!usedAsMovement || !data || !data.GetComponent<AnimationHandler>() || data.GetComponent<AnimationHandler>().currentState != 0)
					{
						...
						{
							rigs[j].AddForce(forceDirection * massM * move.randomMultiplier * forceMultiplier * data.ragdollControl * move.force * move.forceCurve.Evaluate(c), ForceMode.Acceleration);
						}
						if (move.torque != 0f && (bool)rigs[j])
						{
							rigs[j].AddTorque(forceDirection * massM * forceMultiplier * data.ragdollControl * move.torque * move.forceCurve.Evaluate(c), ForceMode.Acceleration);
						}
					}
				}
			}
